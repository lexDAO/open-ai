...
def split_audio_only(video_file):
...
def split_audio_and_video(video_file):
    # Get the base name of the input video file
    base_name = os.path.splitext(os.path.basename(video_file))[0]
...
def split_video_into_tracks(video_file):
    ...
    if choice == 'audio':
        split_audio_only(video_file)
    elif choice == 'both':
        split_audio_and_video(video_file)
    else:
        print("Invalid choice. Please enter 'audio' or 'both'.")

# Example usage
video_file_path = 'video-file-path.mp4'
